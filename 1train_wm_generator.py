...
Target_labels = torch.stack([i * torch.ones(1) for i in list(range(int(opt.wm_classes)))]).expand(
                    int(opt.wm_classes), opt.batch_size+opt.wm_batch_size).permute(1, 0).to(dtype=torch.long, device='cuda')
...
def Train(dataset, feature_r, feature_r_counterpart, recoder, cp_recoder, epoch): #features of the last epoch
    ...
    for fs, labels in tqdm(dataset):
        ...
        Triggers = EmbbedNet(Triggers[:, 0:3 * int(opt.wm_classes), :, :],
                             Triggers[:, 3 * int(opt.wm_classes):6 * int(opt.wm_classes), :, :])
        Triggers = (Triggers) / 255
        Triggers = Triggers.reshape(-1, int(opt.wm_classes), 3, opt.image_size, opt.image_size)
        random_index = torch.randint(int(opt.wm_classes), size=(Triggers.shape[0], wm_batch)).cuda()
